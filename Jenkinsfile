pipeline {
    ...
    stages {
        stage('Setup Runner') {
            steps {
                // Ensure we have the latest version of the deployment tools
                checkout scm

                // Install Python dependencies (lupa, etc.)
        ...
        stage('Execute Deployment') {
            steps {
                script {
                    // We run inside a nix-shell to ensure system dependencies (rsync, curl) exist
                    // Pass the --skip-tests flag if requested by the Jenkins UI
                    def skipFlag = params.SKIP_TESTS ? "--skip-tests" : ""

                    sh """
                        nix-shell -p python3 python3Packages.lupa rsync curl yarn --run \
                        "python3 . --config ./deployment.lua --branch ${env.TARGET_BRANCH} ${skipFlag}"
                    """
                }
            }
        }
