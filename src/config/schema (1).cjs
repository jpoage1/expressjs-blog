...
convict.addFormat({
  name: 'log-level',
  validate(val) {
    const valid = ['error', 'warn', 'event', 'security', 'notice', 'info', 'debug', 'analytics'];
...
const schema = {
  ...
  cleanup: {
    development: {
      max_session_count: { format: 'nat', default: 25, doc: 'Max log sessions in dev' },
      session_retention_hours: { format: Number, default: 1, doc: 'Hours to retain sessions' },
      max_total_size_mb: { format: Number, default: 50, doc: 'Max total log size MB' },
      max_disk_usage_percent: { format: Number, default: 85, doc: 'Disk panic threshold %' },
      cleanup_interval_minutes: { format: 'nat', default: 15, doc: 'Cleanup interval minutes' },
      emergency_cleanup_ratio: { format: Number, default: 0.7, doc: 'Fraction to keep in emergency' },
    },
    production: {
      max_session_count: { format: 'nat', default: 100, doc: 'Max log sessions in production' },
      session_retention_hours: { format: Number, default: 24, doc: 'Hours to retain sessions' },
      max_total_size_mb: { format: Number, default: 200, doc: 'Max total log size MB' },
      max_disk_usage_percent: { format: Number, default: 90, doc: 'Disk panic threshold %' },
      cleanup_interval_minutes: { format: 'nat', default: 60, doc: 'Cleanup interval minutes' },
      emergency_cleanup_ratio: { format: Number, default: 0.5, doc: 'Fraction to keep in emergency' },
    },
  },
